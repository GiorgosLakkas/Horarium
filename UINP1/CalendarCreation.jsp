...
<body>
  ...
  <script>
    ...
    const checkboxes = employeeDropdown.querySelectorAll("input[type='checkbox']");
    ...
    $("assignBtn").addEventListener("click", () => {
      ...
      if (!day || !time || employees.length === 0) {
        alert("Please fill all fields before assigning employees.");
        return;
      }

      const assignment = { day, time, employees };
      let assignments = JSON.parse(
        localStorage.getItem("manager_assignments") || "[]"
      );
      assignments.push(assignment);
      localStorage.setItem("manager_assignments", JSON.stringify(assignments));
